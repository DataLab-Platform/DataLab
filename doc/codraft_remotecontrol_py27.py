...
"""
CodraFT remote controlling class for Python 2.7
"""
...
def get_codraft_xmlrpc_port():
    ...
    fname = osp.join(get_config_dir(), ".CodraFT.ini")
    ini = cp.ConfigParser()
    ini.read(fname)
    try:
...
def test_remote_client():
    """CodraFT Remote Client test"""
    codraft = RemoteClient()
    codraft.connect()
    data = np.array([[3, 4, 5], [7, 8, 0]], dtype=np.uint16)
    codraft.add_image("toto", data)
