...
**Web API localhost bypass**
    When enabled, connections from localhost (127.0.0.1) to the Web API do not require
    authentication. This simplifies notebook integration when DataLab-Kernel runs on
    the same machine. Disabled by default for security.

    .. note::

        When this option is enabled, any application running on your local machine can
        access the DataLab Web API without a token. Only enable this if you understand
