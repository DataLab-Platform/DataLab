...
def __compute_1_to_1_operations(panel: SignalPanel | ImagePanel, number: int) -> None:
    ...
    panel.processor.run_feature("gaussian_filter", sigima_.param.GaussianParam())
    panel.processor.run_feature("moving_average", sigima_.param.MovingAverageParam())
    panel.processor.run_feature("moving_median", sigima_.param.MovingMedianParam())
    ...
    panel.processor.run_feature(
        "astype",
        sigima_.param.DataTypeIParam.create(dtype_str="float64"),
    )
...
def compute_common_operations(panel: SignalPanel | ImagePanel) -> None:
    ...
    const_oper_param = sigima_.param.ConstantParam.create(value=2.0)
    for const_oper in (
        "addition_constant",
        "difference_constant",
        "product_constant",
    ...
    param = sigima_.param.ConstantParam()
    param.value = 2.0
...
def run_signal_computations(
    ...
    panel.processor.run_feature("power", sigima_.param.PowerParam.create(power=2))
    panel.processor.run_feature("reverse_x")
    panel.processor.run_feature("reverse_x")

    # Test filter methods
    for filter_func_name, paramclass in (
        ("lowpass", sigima_.param.LowPassFilterParam),
        ("highpass", sigima_.param.HighPassFilterParam),
        ("bandpass", sigima_.param.BandPassFilterParam),
        ("bandstop", sigima_.param.BandStopFilterParam),
    ):
    ...
    param = sigima_.param.WindowingParam()
    for method_value, _method_name in param.methods:
    ...
    param = sigima_.param.XYCalibrateParam.create(a=1.2, b=0.1)
    ...
    y = x * 0.0
    sig2 = sigima_.obj.create_signal("X values for interpolation", x, y)
    panel.add_object(sig2)

    # Test interpolation
    # pylint: disable=protected-access
    for method_choice_tuple in sigima_.param.InterpolationParam.methods:
    ...
    for mode, dx, nbpts in (("dx", 0.1, 10), ("nbpts", 0.0, 100)):
        panel.objview.set_current_object(sig1)
        param = sigima_.param.ResamplingParam.create(
    ...
    panel.objview.set_current_object(sig1)
    # pylint: disable=protected-access
    for method_choice_tuple in sigima_.param.DetrendingParam.methods:
        param = sigima_.param.DetrendingParam.create(method=method_choice_tuple[0])
        panel.processor.run_feature("detrending", param)

    # Test histogram
    panel.objview.set_current_object(sig1)
    param = sigima_.param.HistogramParam.create(bins=100)
...
def run_image_computations(
    ...
    newparam = sigima_.obj.NewImageParam.create(height=data_size, width=data_size)
    ...
    param = sigima_.param.ZCalibrateParam.create(a=1.2, b=0.1)
    panel.processor.run_feature("calibration", param)
    param = sigima_.param.DenoiseTVParam()
    panel.processor.run_feature("denoise_tv", param)
    param = sigima_.param.DenoiseBilateralParam()
    panel.processor.run_feature("denoise_bilateral", param)
    param = sigima_.param.DenoiseWaveletParam()
    ...
    param = sigima_.param.AdjustGammaParam.create(gamma=0.5)
    panel.processor.run_feature("adjust_gamma", param)
    param = sigima_.param.AdjustLogParam.create(gain=0.5)
    panel.processor.run_feature("adjust_log", param)
    param = sigima_.param.AdjustSigmoidParam.create(gain=0.5)
    panel.processor.run_feature("adjust_sigmoid", param)
    param = sigima_.param.EqualizeHistParam()
    panel.processor.run_feature("equalize_hist", param)
    param = sigima_.param.EqualizeAdaptHistParam()
    panel.processor.run_feature("equalize_adapthist", param)
    param = sigima_.param.RescaleIntensityParam()
    ...
    param = sigima_.param.ButterworthParam.create(order=2, cut_off=0.5)
    panel.processor.run_feature("butterworth", param)

    param = sigima_.param.CannyParam()
    panel.processor.run_feature("canny", param)

    # Test threshold methods
    ima2 = create_sincos_image(newparam, extra_param=addparam)
    panel.add_object(ima2)
    param = sigima_.param.ThresholdParam()
    for method_value, _method_name in param.methods:
        panel.objview.set_current_object(ima2)
        param = sigima_.param.ThresholdParam.create(method=method_value)
    ...
    param = sigima_.param.LogP1Param.create(n=1)
    ...
    param = sigima_.param.RotateParam.create(angle=5.0)
    for boundary in param.boundaries[:-1]:
        param.mode = boundary
        panel.processor.run_feature("rotate", param)

    param = sigima_.param.ResizeParam.create(zoom=1.3)
    ...
    param = sigima_.param.BinningParam.create(sx=2, sy=2, operation="average")
