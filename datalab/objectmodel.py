...
#: Metadata key holding the canonical title generated by a processing operation.
#: This is a public key so it is copied with the object and serialized to files.
COMPUTED_TITLE_KEY = "computed_title"


def get_computed_title(obj: SignalObj | ImageObj) -> str | None:
    """Return the canonical title generated by processing, if available."""
    value = obj.metadata.get(COMPUTED_TITLE_KEY)
    return value if isinstance(value, str) else None


def set_computed_title(obj: SignalObj | ImageObj, title: str) -> None:
    """Store the canonical title generated by processing."""
    obj.metadata[COMPUTED_TITLE_KEY] = title
...
def patch_title_with_ids(
    ...
    try:
        ...
        set_computed_title(dst_obj, dst_obj.title)
...
class ObjectModel:
    ...
    def __get_reference_replacement(
        obj_uuid: str, short_id: str, use_short_ids: bool
    ) -> tuple[str, str]:
        """Return the old/new pair for a title reference substitution."""
        if use_short_ids:
            return short_id, obj_uuid
        return obj_uuid, short_id
    ...
    def __freeze_deleted_object_refs(
        ...
        for dependent in dependents:
            self.__freeze_dependent_refs(dependent, token_re, prefix, frozen_title)

    def __freeze_dependent_refs(
        self,
        dependent: SignalObj | ImageObj | ObjectGroup,
        token_re: re.Pattern,
        prefix: str,
        frozen_title: str,
    ) -> None:
        ...
        if not (in_title or in_computed_title or in_registry):
            return
    ...
    def __replace_local_references(
        self,
        mapping: dict[str, str],
        other_objects: tuple[SignalObj | ImageObj] | None,
        use_short_ids: bool,
    ) -> None:
        """Replace references in local object and group titles."""
        objects = self._objects.values()
        if other_objects is not None:
            objects = list(objects) + list(other_objects)
        for obj in objects:
            for obj_uuid, short_id in mapping.items():
                old, new = self.__get_reference_replacement(
                    obj_uuid, short_id, use_short_ids
                )
                self.__replace_in_object_titles(obj, old, new)
        for group in self._groups:
            for obj_uuid, short_id in mapping.items():
                old, new = self.__get_reference_replacement(
                    obj_uuid, short_id, use_short_ids
    ...
    def __replace_sibling_references(
        ...
        """Replace references in sibling model titles."""
        for sibling_item in self.__iter_sibling_titled_items():
            for obj_uuid, short_id in mapping.items():
                old, new = self.__get_reference_replacement(
                    obj_uuid, short_id, use_short_ids
                )
    ...
    def __replace_registry_references(
        ...
        mapping: dict[str, str],
        use_short_ids: bool,
        other_objects: tuple[SignalObj | ImageObj] | None = None,
    ) -> None:
        """Replace live references embedded in deleted-source registries."""
        for registry in self.__iter_registries(other_objects):
            for token, title in registry.items():
                for obj_uuid, short_id in mapping.items():
                    old, new = self.__get_reference_replacement(
                        obj_uuid, short_id, use_short_ids
                    )
                    title = title.replace(old, new)
                registry[token] = title
    ...
    def replace_short_ids_by_uuids_in_titles(
        ...
        self.__replace_local_references(mapping, other_objects, use_short_ids=True)
        ...
        self.__replace_sibling_references(mapping, use_short_ids=True)
        # Keep live short IDs embedded in frozen (deleted-reference) titles in sync:
        self.__replace_registry_references(mapping, True, other_objects)
    ...
    def replace_uuids_by_short_ids_in_titles(self) -> None:
        ...
        """
        mapping = self.__get_group_object_mapping_to_shortid()
        self.__replace_local_references(mapping, None, use_short_ids=False)
        ...
        self.__replace_sibling_references(mapping, use_short_ids=False)
        # Restore live short IDs embedded in frozen (deleted-reference) titles:
        self.__replace_registry_references(mapping, use_short_ids=False)
        ...
        for obj in self._objects.values():
            self.__replace_missing_uuids_in_object_titles(obj, pattern)
        for group in self._groups:
            for obj in group:
