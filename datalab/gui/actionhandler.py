...
class BaseActionHandler(metaclass=abc.ABCMeta):
    ...
    def create_first_actions(self):
        ...
        with self.new_category(ActionCategory.EDIT):
            ...
            self.new_action(
                _("Reset to computed title"),
                icon_name="restore.svg",
                tip=_("Restore the title generated by the processing operation"),
                triggered=self.panel.reset_selected_title_to_computed,
                select_condition=self.has_resettable_computed_title,
                context_menu_pos=-1,
            )
            self.new_action(
                _("New group..."),
