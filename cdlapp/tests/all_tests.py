...
import cdlapp
from cdlapp import __version__, config
from cdlapp.config import Conf
from cdlapp.utils.conf import Option
from cdlapp.utils.tests import TST_PATH
...
def run_all_tests(args="", contains="", timeout=None, other_package=None):
    ...
    testmodules, testnb = get_test_modules(cdlapp, contains=contains)
    ...
    for idx, testmodule in enumerate(testmodules):
        rpath = osp.relpath(testmodule.path, osp.dirname(cdlapp.__file__))
