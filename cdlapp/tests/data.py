...
def get_test_signal(filename: str) -> cdlapp.obj.SignalObj:
    """Return test signal
    ...
    """
    return cdlapp.obj.read_signal(get_test_fnames(filename)[0])


def get_test_image(filename: str) -> cdlapp.obj.ImageObj:
    """Return test image
    ...
    """
    return cdlapp.obj.read_image(get_test_fnames(filename)[0])


def create_paracetamol_signal(
    size: int | None = None, title: str | None = None
) -> cdlapp.obj.SignalObj:
    ...
    Returns:
        SignalObj: Signal object
    """
    obj = cdlapp.obj.read_signal(get_test_fnames("paracetamol.txt")[0])
...
def add_gaussian_noise_to_signal(
    signal: cdlapp.obj.SignalObj, p: GaussianNoiseParam | None = None
) -> None:
    """Add Gaussian (Normal-law) random noise to data

    Args:
        signal (cdlapp.obj.SignalObj): Signal object
...
def create_noisy_signal(
    ...
    newparam: cdlapp.obj.NewSignalParam | None = None,
    addparam: cdlapp.obj.GaussLorentzVoigtParam | None = None,
    title: str | None = None,
    ...
    Args:
        noiseparam (GaussianNoiseParam | None): Noise parameters. Default:
            None: No noise
        newparam (cdlapp.obj.NewSignalParam | None): New signal parameters.
            Default: Gaussian, size=500, xmin=-10, xmax=10
        addparam (cdlapp.obj.GaussLorentzVoigtParam | None): Additional parameters.
    ...
    Returns:
        cdlapp.obj.SignalObj: Signal object
    """
    if newparam is None:
        newparam = cdlapp.obj.NewSignalParam()
        newparam.stype = cdlapp.obj.SignalTypes.GAUSS
    if title is not None:
        newparam.title = title
    newparam.title = "Test signal (noisy)" if newparam.title is None else newparam.title
    if addparam is None:
        addparam = cdlapp.obj.GaussLorentzVoigtParam()
    if noised is not None and noiseparam is None:
        noiseparam = GaussianNoiseParam()
        noiseparam.sigma = 5.0
    sig = cdlapp.obj.create_signal_from_param(newparam, addparam)
...
def get_laser_spot_data() -> list[np.ndarray]:
    ...
    return [zgauss + znoise] + [
        cdlapp.obj.read_image(fname).data for fname in get_test_fnames("*.scor-data")
    ]
...
def __set_default_size_dtype(
    p: cdlapp.obj.NewImageParam | None = None,
) -> cdlapp.obj.NewImageParam:
    """Set default shape and dtype

    Args:
        p (cdlapp.obj.NewImageParam | None): Image parameters. Defaults to None.
            If None, a new object is created.

    Returns:
        cdlapp.obj.NewImageParam: Image parameters
    """
    if p is None:
        p = cdlapp.obj.NewImageParam()
    p.height = 2000 if p.height is None else p.height
    p.width = 2000 if p.width is None else p.width
    p.dtype = cdlapp.obj.ImageDatatypes.UINT16 if p.dtype is None else p.dtype
    return p


def add_gaussian_noise_to_image(
    image: cdlapp.obj.ImageObj, param: cdlapp.obj.NormalRandomParam
) -> None:
    """Add Gaussian noise to image

    Args:
        src (cdlapp.obj.ImageObj): Source image
        param (cdlapp.obj.NormalRandomParam): Parameters for the normal distribution
    """
    newparam = cdlapp.obj.new_image_param(
        height=image.data.shape[0],
        width=image.data.shape[1],
        dtype=cdlapp.obj.ImageDatatypes.from_dtype(image.data.dtype),
        itype=cdlapp.obj.ImageTypes.NORMALRANDOM,
    )
    noise = cdlapp.obj.create_image_from_param(newparam, param)
...
def create_ring_image(p: RingParam | None = None) -> cdlapp.obj.ImageObj:
...
def create_annotated_image(title: str | None = None) -> cdlapp.obj.ImageObj:
